...
Oranges Music 是魔改的ceru Music https://github.com/timeshiftsauce/CeruMusic ，是基于 Electron 和 Vue 开发的跨平台桌面音乐播放器工具，**仅提供插件运行框架与播放功能**，不直接存储、提供任何音乐源文件。用户需通过自行选择、安装合规插件获取音乐相关数据，项目旨在为开发者提供桌面应用技术实践与学习案例，为用户提供合规的音乐播放工具框架。
...
- **Pinia**：状态管理工具
- **Vite**：快速的前端构建工具
- **OrangesPlugins**：音乐插件运行环境（仅提供框架，不包含默认插件）
...
-**明确要求插件开发者需遵守数据来源平台的用户协议与版权法**，禁止开发、传播获取非公开数据的插件。
...
- “GNU AGPL v3.0”：指 Oranges Music（橘子音乐）桌面播放器框架及源代码，不包含任何第三方插件或音乐数据。
